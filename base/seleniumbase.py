...
class SeleniumBase:
    ...
    def __get_selenium_by(self, find_by: str) -> dict:
        """Return a dictionary, where Keys are Strings representing a search locator strategies and Values are
        related By class values"""
    ...
    def is_clickable(self, element: WebElement) -> WebElement:
        """Waiting on an element to be clickable"""
        return self.__wait.until(ec.element_to_be_clickable(element), f'Unable to click {element.get_attribute("class")}')